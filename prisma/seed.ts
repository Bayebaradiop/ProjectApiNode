...
async function main() {
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(`
      INSERT IGNORE INTO PromoReferentiel (promoId, referentielId) VALUES
        (${promo2024.id}, ${refDevWeb.id}),
        (${promo2025.id}, ${refData.id});
    `);
  ...
  const promoReferentielCount = await prisma.$queryRawUnsafe<{ count: number }[]>('SELECT COUNT(*) as count FROM PromoReferentiel');
  console.log(`   - ${promoReferentielCount[0]?.count ?? 0} associations promo/référentiel`);
